    ...
    if the_fact_finding.valid?
      ...
      msg = Message.new
        ...
        msg.message= <<~HTML
          ...
          <p>In accordance with the Collective Bargaining Agreement (CBA), Article 18, Section B.1, a Fact-Finding Meeting has been scheduled for you to attend on <strong> #{params.fetch("query_date2")} at #{params.fetch("query_time2")}</strong> Base Local Time to discuss #{params.fetch("query_to_discuss")}.  Please confirm that you have received this FFM notification by selecting a response option. Note that because you are only entitled to one reschedule, your options are only to accept the meeting or decline to attend. </p>

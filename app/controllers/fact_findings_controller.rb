  ...
  def create
    ...
    if the_fact_finding.valid?
      ...
      msg = Message.new(
        ...
        message: <<~HTML
          <br>
          <p>In accordance with the Collective Bargaining Agreement (CBA), Article 18, Section B.1, a Fact-Finding Meeting has been scheduled for you to attend on #{params.fetch("query_date")} at #{params.fetch("query_time")} Base Local Time to discuss #{params.fetch("query_to_discuss")}.  Please confirm that you have received this FFM notification by marking this message as read.</p>
